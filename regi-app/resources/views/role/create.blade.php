        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <h1 class="text-center fw-bold"> Create Roles  </h1>
                    
                    <div class="flex items-center justify-center h-screen mt-5 bg-white-100 ">

                        <form method="POST" action="{{route('roles.store')}}" class="bg-primary p-5 rounded w-50" >       
                            @csrf
...
{{-- 
                        <div class="mt-2">
                            @foreach ($permissions as $per)
                             <label for="" type="checkbox" name="permissions [{{$per->name}}]" value="{{$per->name}}">{{$per->name}}</label>   
                            @endforeach
                        </div> --}}
